...
thresholds_to_try = {
	...
	"sevenday_avg_newICU":[10*(i+1) for i in range(40)] # Revisit this]
}
...
def ICU_admissions_trigger_policy(experiment_params, thresholds, activity_levels, plot=False):
	
	# Define the high and low activity leves
	...
	result = {
		"heuristic":"ICU_admissions_trigger",
...
# Finding best parameter settings for ICU admissions trigger policy
partial_results = []
all_results = []
						...
						for sevenday_avg_newICU_t in thresholds_to_try["sevenday_avg_newICU"]:
							for act_H_t in lockdowns_to_try["high_activity"]:
								for act_L_t in lockdowns_to_try["low_activity"]:
									...
									partial_result = ICU_admissions_trigger_policy(experiment_params, thresholds, activity_levels)
...
for r in all_results:
	...
	thresholds = {
		...
		"sevenday_avg_newICU":r["sevenday_avg_newICU"],
	}	

	ICU_admissions_trigger_policy(experiment_params, thresholds, activity_levels, plot=True)


pd.DataFrame(all_results).to_excel(f"ICU_admissions_trigger_simulations-{simulation_params['days']}-days.xlsx")
pd.DataFrame(partial_results).to_excel(f"ICU_admissions_trigger_partial_simulations-{simulation_params['days']}-days.xlsx")
