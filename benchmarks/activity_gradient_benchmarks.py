...
def gradient_descent(experiment_params, quar_freq, plot=False):
	...
	result = {
		"lockdown_heuristic":"activity_gradient",
		"groups":groups,
		"experiment_params":{
	...
	with open("results/"+constant_gradients_filename+".yaml") as file:
		initial_alphas = yaml.load(file, Loader=yaml.FullLoader)["policy"]
	...
	for i,t in enumerate(intervention_times):
		for k,act in enumerate(rel_activities):
			x0[i*len(rel_activities) + k] = initial_alphas[t][age_groups[0]][act]
	...
	def simulate(x):
		...
		for i,t in enumerate(intervention_times):
			for k,act in enumerate(rel_activities):
				x_lockdown[i,:,k] = x[i*len(rel_activities) + k]
	...
	full_bounds = Bounds(
		np.zeros(len(intervention_times)*len(rel_activities)),
	...
	for i,t in enumerate(intervention_times):
		for k,act in enumerate(rel_activities):
			x_lockdown[i,:,k] = result_lockdown.x[i*len(rel_activities) + k]
