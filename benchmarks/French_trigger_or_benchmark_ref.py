...
thresholds_to_try = {
	"icu_utilization_rate":[0.2*(i+1) for i in range(5)], # Revisit this
	"incidence_rate":[0.0006*(i+1) for i in range(5)], # Revisit this
	"incidence_rate_old":[0.0006*(i+1) for i in range(5)], # Revisit this
}
...
def French_trigger_or_policy(experiment_params, thresholds, activity_levels):


	result_policy = {
		"lockdown_heuristic":"French_trigger_or",
		"groups":groups,
		"experiment_params":{
	...
	for t in range(simulation_params['time_periods']):
		...
		if (icu_utilization_rate>thresholds["icu_utilization_rate"]) or (incidence_rate>thresholds["incidence_rate"]) or (incidence_rate_old>thresholds["incidence_rate_old"]):
...
def French_trigger_or_policy_fast(experiment_params, thresholds, activity_levels):
	...
	for t in range(simulation_params['time_periods']):
		...
		if (t==0):
			initial_population = np.sum(state[:,cont.index("N")])
			initial_population_old = np.sum(state[6:,cont.index("N")])
		...
		newInf = np.sum(fastModel.get_new_infections())
		newInf_old = np.sum(fastModel.get_new_infections_old())


		sevendays_newInf.append(newInf)
		sevendays_newInf_old.append(newInf_old)

		# Drop first entry if larger than 7
		if len(sevendays_newInf)>7:
			sevendays_newInf = sevendays_newInf[1:8]
		incidence_rate = np.sum(sevendays_newInf)/initial_population/len(sevendays_newInf)
		
		# Drop first entry if larger than 7
		if len(sevendays_newInf_old)>7:
			sevendays_newInf_old = sevendays_newInf_old[1:8]
		incidence_rate_old = np.sum(sevendays_newInf_old)/initial_population_old/len(sevendays_newInf_old)
				...
				for testing in params_to_try["testing"]:
					for eta in params_to_try["eta"]:
						...
						total_iterations = (
							...
							len(thresholds_to_try["incidence_rate"])*
							len(thresholds_to_try["incidence_rate_old"])*
						...
						for icu_util_rate_t in thresholds_to_try["icu_utilization_rate"]:
							for incidence_rate_t in thresholds_to_try["incidence_rate"]:
								for incidence_rate_old_t in thresholds_to_try["incidence_rate_old"]:
									for act_H_t in lockdowns_to_try["high_activity"]:
										for act_L_t in lockdowns_to_try["low_activity"]:
												if act_L_t<act_H_t:
													thresholds = {
														"incidence_rate":incidence_rate_t,
														"incidence_rate_old":incidence_rate_old_t,
													...
													partial_result = French_trigger_or_policy_fast(experiment_params, thresholds, activity_levels)
													# other_result = French_trigger_or_policy(experiment_params, thresholds, activity_levels)
													# print(partial_result)
													# print(other_result["results"])
						...
						result_policy = French_trigger_or_policy(experiment_params, best_result["thresholds"], best_result["activity_levels"])
