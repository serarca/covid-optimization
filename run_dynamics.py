...
for i,group in enumerate(groups):
	...
	plt.plot(range(0,int(simulation_params['time_periods'])), np.array(re_change_order(m_tests_vec)[group])+max(m_tests,a_tests)/100, label="M Tests")
	plt.plot(range(0,int(simulation_params['time_periods'])), re_change_order(a_tests_vec)[group], label="A Tests")
	plt.ylim(-max(m_tests,a_tests)/10,max(m_tests,a_tests)+max(m_tests,a_tests)/10)
	plt.legend(loc='upper right')
