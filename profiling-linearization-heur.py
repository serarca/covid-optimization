...
def run_lin_heur_and_pickle_dynModel(delta, xi, icus, tests, n_days, region, test_freq, lockdown_freq):
    ...
    pickle.dump(dynModel_linearization_heur,open(f"linearization_heuristic_dyn_models/dynModel_linHeur_n_days={n_days}_deltas={delta}_xi={xi}_icus={icus}_maxTests={tests}_testFreq={test_freq}_lockFreq={lockdown_freq}.p","wb"))
...
def run_dyn_model_with_no_bouncing_and_pickle(pickled_dyn_model):
    ...
    pickle.dump(dynModel,open(f"linearization_heuristic_dyn_models/dynModel_linHeur_Prop_Bouncing_n_days={n_days}_deltas={delta}_xi={xi}_icus={icus}_maxTests={tests}_testFreq={dynModel.experiment_params['test_freq']}_lockFreq={dynModel.experiment_params['lockdown_freq']}.p","wb"))
                ...
                for tests in params_to_try["tests"]:
                    for heur in ["","_Prop_Bouncing"]:
                        for test_freq, lockdown_freq in params_to_try['frequencies']:
                            ...
                            dynModel = pickle.load(open(f"linearization_heuristic_dyn_models/dynModel_linHeur{heur}_n_days={n_days}_deltas={delta}_xi={xi}_icus={icus}_maxTests={tests}_testFreq={test_freq}_lockFreq={lockdown_freq}.p","rb"))
    ...
    for delta in params_to_try["delta_schooling"]:
        for xi in params_to_try["xi"]:
            for icus in params_to_try["icus"]:
                for tests in params_to_try["tests"]:
                    for test_freq, lockdown_freq in params_to_try['frequencies']:
                        pickled_dyn_model = f"linearization_heuristic_dyn_models/dynModel_linHeur_n_days={n_days}_deltas={delta}_xi={xi}_icus={icus}_maxTests={tests}_testFreq={test_freq}_lockFreq={lockdown_freq}.p"
