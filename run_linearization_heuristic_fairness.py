...
def run_lin_heur_and_save_yaml(delta, xi, icus, mtests, atests, n_days, region, test_freq, lockdown_freq, econ, init, eta, groups, start_day, trust_region_radius, max_inner_iterations_mult, initial_uhat, optimize_bouncing, scaling, money_scaling, targetActivities, targetGroups, targetTests, fairness, deltaFair):
    ...
    dynModel = run_linearization_heuristic(simulation_params, experiment_params, start_day, trust_region_radius, max_inner_iterations_mult, initial_uhat, optimize_bouncing, targetActivities, targetGroups, targetTests, fairness, deltaFair)

    result = {
            "lockdown_heuristic":f"linearization_heuristic_optBouncing={optimize_bouncing}_initial_uhat={initial_uhat}_targetGroups={targetGroups}_targetAct={targetActivities}_targetTests={targetTests}_fairnessOne={fairness}",
            "groups":groups,
            "experiment_params":{
                "delta_schooling":delta,
...
def run_lin_heur_and_pickle_dynModel(delta, xi, icus, tests, n_days, region, test_freq, lockdown_freq, econ, init, eta, groups, start_day, targetActivities, targetGroups, targetTests, fairness, deltaFair):
    ''' Runs the linearization heuristic with the experiment parameters passed as arguments and saves the resulting dynamical model as a pickle object.'''
    ...
    dynModel_linearization_heur = run_linearization_heuristic(simulation_params_linearization, experiment_params, start_day, targetActivities, targetGroups, targetTests, fairness, deltaFair)
...
def run_linearization_heuristic(simulation_params, experiment_params, start_day, trust_region_radius, max_inner_iterations_mult, initial_uhat, optimize_bouncing, targetActivities, targetGroups, targetTests, fairness, deltaFair):
    ...
    linearization.run_heuristic_linearization(dynModel, trust_region_radius, max_inner_iterations_mult, initial_uhat, optimize_bouncing, targetActivities, targetGroups, targetTests, fairness, deltaFair)
