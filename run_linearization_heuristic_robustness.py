...
def main():
    ...
    scaling_econ_param(scaling, money_scaling, groups, random_instance)
    scaling_fitted(scaling, money_scaling, groups, random_instance)
    scaling_init(scaling, groups, random_instance)
...
def run_lin_heur_and_save_yaml(delta, xi, icus, mtests, atests, n_days, region, test_freq, lockdown_freq, econ, init, eta, groups, start_day, trust_region_radius, max_inner_iterations_mult, initial_uhat, optimize_bouncing, scaling, money_scaling, random_instance, nu, gamma):
...
def run_linearization_heuristic(simulation_params, experiment_params, start_day, trust_region_radius, max_inner_iterations_mult, initial_uhat, optimize_bouncing, random_instance, nu, gamma):
    ...
    with open("parameters/simulations/"+simulation_params["region"]+f"_{random_instance}.yaml","r") as file:
        # The FullLoader parameter handles the conversion from YAML
        # scalar values to Python the dictionary format
        while file is None:
            print("Failed reading fitted param")
            file = open("parameters/simulations/"+simulation_params["region"]+f"_{random_instance}.yaml","r")
        ...
        # Read initialization
    with open(f"parameters/simulations/{simulation_params['init']}_{random_instance}.yaml","r") as file:
        # The FullLoader parameter handles the conversion from YAML
        # scalar values to Python the dictionary format
        while file is None:
            print("Failed reading init param")
            file = open(f"parameters/simulations/{simulation_params['init']}_{random_instance}.yaml","r")
    ...
    with open(f"parameters/simulations/{simulation_params['econ']}_{random_instance}.yaml","r") as file:
        while file is None:
            print("Failed reading econ param")
            file = open(f"parameters/simulations/{simulation_params['econ']}_{random_instance}.yaml","r")
...
# def scaling_econ_param(scaling, money_scaling, random_instance):
#     # Import data
#     old_econ = yaml.load(open( f"parameters/simulations/econ_{random_instance}.yaml", "rb" ),Loader=yaml.FullLoader)
#     # scaling = 1000.0
...
#     with open(f'parameters/simulations/econ-scaled_{random_instance}.yaml', 'w') as file:
#         yaml.dump(scaled_econ, file)




def scaling_econ_param(scaling, money_scaling, groups, random_instance):
    # Import data
    if groups == "all":
        old_econ = yaml.load(open( f"parameters/simulations/econ_{random_instance}.yaml", "rb" ),Loader=yaml.FullLoader)
...
def scaling_init(scaling, groups, random_instance):
    # Import data
    if groups == "all":
        old_init = yaml.load(open( f"parameters/simulations/oct21_{random_instance}.yaml", "rb" ), Loader=yaml.FullLoader)
    ...
    if groups == "all":
        with open(f'parameters/simulations/oct21-scaled_{random_instance}.yaml', 'w') as file:
...
def scaling_fitted(scaling, money_scaling, groups, random_instance):

    if groups == "all":
        # Import data
        old_fitted = yaml.load(open( f"parameters/simulations/fitted_{random_instance}.yaml", "rb" ), Loader=yaml.FullLoader)
    # scaling = 1000.0
    ...
    if groups == "all":
        with open(f'parameters/simulations/fitted-scaled_{random_instance}.yaml', 'w') as file:
